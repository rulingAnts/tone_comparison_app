 ...
 *  - Reliable LUFS normalization (two-pass loudnorm targeting -16 LUFS with limiter)
...
function buildFilters({ autoTrim, autoNormalize, targetLUFS, limiterCeiling, silenceThreshold, minSilenceMs, twoPass }) {
  ...
  // Skip loudnorm in first chain if two-pass; we'll inject it after measurement.
...
function mapOutputPath(inFile, inputRoot, outputRoot, outputFormat) {
  ...
  // Replace the last extension generically (handles mp3, m4a, wav, flac, etc.)
  if (/\.[^\/\.]+$/.test(rel)) {
    return path.join(outputRoot, rel.replace(/\.[^\/\.]+$/, newExt));
  }
  // If no extension, append one
...
async function normalizeFile(inputFile, outputFile, opts = {}) {
  const {
    ...
    twoPass = true, // enable robust two-pass loudnorm by default
  } = opts;
  ...
  // Build final filter chain (include loudnorm with measured params if two-pass)
  let finalFilter = filters;
  if (autoNormalize && twoPass) {
    const limitFilter = `alimiter=limit=${limiterCeiling}`; // already included in filters end; we'll rebuild to ensure order
    const I = targetLUFS;
    if (loudnormStats && typeof loudnormStats.input_i === 'string') {
      // Stats fields documented by ffmpeg loudnorm filter
      const measured_I = loudnormStats.input_i;
      const measured_LRA = loudnormStats.input_lra;
      const measured_TP = loudnormStats.input_tp;
      const measured_thresh = loudnormStats.input_thresh;
      const offset = loudnormStats.target_offset;
      const loudnormFilter = `loudnorm=I=${I}:TP=-1.0:LRA=11:measured_I=${measured_I}:measured_LRA=${measured_LRA}:measured_TP=${measured_TP}:measured_thresh=${measured_thresh}:offset=${offset}:linear=true:print_format=none`;
      // Remove any existing alimiter from first chain to avoid duplication
      const pieces = filters.split(',').filter(f => !/^alimiter=/.test(f));
      finalFilter = [...pieces, loudnormFilter, limitFilter].join(',');
    } else {
      // Fallback: no stats parsed, use single-pass loudnorm
