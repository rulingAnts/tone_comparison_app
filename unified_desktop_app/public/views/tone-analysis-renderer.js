// ============================================================================
// ADAPTED FOR UNIFIED APP - Load bundle from path instead of file picker
// ============================================================================

// Prevent re-initialization if script loads multiple times
if (!window.toneAnalysisInitialized) {
  window.toneAnalysisInitialized = true;

  const ipcRenderer = {
    invoke: (channel, ...args) => window.electronAPI.invoke(channel, ...args),
    send: (channel, ...args) => {
      // Stub - unified app doesn't need send for most operations
      console.log('[ipcRenderer.send stub]', channel, args);
    }
  };
...
// Initialize - wrapped in function so it can be called when view loads
...
// Call initialization when DOM is ready (for standalone desktop_matching_app)
// For unified app, this will be called by initializeToneAnalysisView
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', initializeRenderer);
} else {
  // DOM already loaded (unified app case)
  setTimeout(initializeRenderer, 0);
}
