; Custom NSIS installer script for Tone Matching Desktop
; Handles cleanup with user confirmation

!macro customInit
  ; Check if a previous version exists BEFORE installation starts
  IfFileExists "$INSTDIR\Tone Matching Desktop.exe" show_warning no_previous_install
  
  show_warning:
    MessageBox MB_YESNO|MB_ICONEXCLAMATION \
      "A previous installation of Tone Matching Desktop was detected.$\n$\n\
  ...
  RMDir /r "$LOCALAPPDATA\Tone Matching Desktop"
  RMDir /r "$LOCALAPPDATA\tone-matching-desktop"
  RMDir /r "$APPDATA\Tone Matching Desktop"
  RMDir /r "$APPDATA\tone-matching-desktop"
  RMDir /r "$TEMP\Tone Matching Desktop"
!macroend
